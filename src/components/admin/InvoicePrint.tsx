    ...
    <tbody>
      ${items.map((item: any) => `
        <tr>
          ...
          <td>${item.color}</td>
          <td>${item.quantity}</td>
          <td style="text-align:right">${Number(item.price).toFixed(3)} AED</td>
          <td style="text-align:right">${(item.price * item.quantity).toFixed(3)} AED</td>
        </tr>
      `).join('')}
    </tbody>
  </table>
  <div class="totals">
    <div class="row"><span>Subtotal:</span><span>${Number(order.total).toFixed(3)} AED</span></div>
    <div class="row total-row"><span>Total:</span><span>${Number(order.total).toFixed(3)} AED</span></div>
  </div>
  <div class="footer">
    <p>Thank you for shopping with SRK Collection · Dubai</p>
  </div>
</body>
</html>`;
...
<body>
  <div class="slip">
    ...
    <div class="total-amount">${Number(order.total).toFixed(3)} AED</div>
    <div class="footer">
